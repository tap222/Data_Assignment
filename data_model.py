...
print('Avg of time for valid games:',grp['time'].sum()/grp['time'].count())
print('Total  Valid session:',comb.groupby(['gameid']).size())
game_ttl = pd.concat([st,inval_game],axis=0)
print('Total no of sessions:',game_ttl.groupby(['gameid']).size())
